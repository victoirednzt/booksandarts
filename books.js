const books = [

    {
        id: 1,

        title: "Juno loves Legs",

        author: "Karl Geary",

        genre: "Roman",

        rating: 3.5,

        favorite: true,

        cover: "images/juno-legs.jpg",

        keywords: [
            "Irlande",
            " Education catholique (oups)",
            " #Friendship"
        ],

        description:
            "roman irlandais, amitié entre deux loulous dans un contexte de full galère. give un peu sorj chalandon pour les initiés #zoé",

        date:
            "Lu en Juillet 2026 sur la reco de J. Bordeaux"
    },



    {
        id: 2,

        title: "Everything Was Beautiful, and Nothing Hurt",

        author: "Ben Reeves",

        genre: "Roman",

        rating: 4.5,

        favorite: true,

        cover: "images/everything-beautiful.jpg",

        keywords: [
            "La mort en narrateur",
            " Lyrique et cynique",
            " TW: un poil triste quand même mais très beau!"
        ],

        description:
            "Ce livre nous plonge dans le récit de la Mort, embodied par un narrateur qui est un boug d'une trentaine d'années et qui rend visite aux gens quelques minutes avant qu'ils décèdent. Il se lie d'amitié avec sa voisine, mère seule et ses deux filles. C'est beau! De petites digressions poétiques entrecoupent l'histoire, avec justess",

        date:
            "Lu en Juillet 2026"
    },



    {
        id: 3,

        title: "The Chosen (L'élu)",

        author: "Chaim Potok",

        genre: "Roman",

        rating: 3,

        favorite: true,

        cover: "images/the-chosen.jpg",

        keywords: [
            "Religion",
            "Amitié",
            "Transmission"
        ],

        description:
            "Deux adolescents juifs issus de milieux différents (orthodoxe ++ et un peu moins) développent une amitié profonde dans le Brooklyn des années 1940.Juste avant la création d'Israël qui change pas mal de choses, mais je m'attendais à ce qu'il en fasse un sujet sur une plus grande partie du livre",


        date:
            "Lu en Juillet 2026. je peux prêter le livre (fr)"
    },



    {
        id: 4,

        title: "The Brief Wondrous Life of Oscar Wao",

        author: "Junot Díaz",

        genre: "Roman",

        rating: 4,

        favorite: true,

        cover: "images/oscar-wao.jpg",

        keywords: [
            "Pulitzer Prize",
            "  Diaspora (rép dominicaine)",
            "  Slang Espagnol"
        ],

        description:
            "L'histoire d'Oscar, un jeune Dominicain-Américain passionné de science-fiction, à travers une fresque familiale mêlant humour, tragédie et histoire politique.",

        date:
            "Lu en Juillet 2026"
    },



    {
        id: 5,

        title: "All the Light We Cannot See",

        author: "Anthony Doerr",

        genre: "Roman",

        rating: 4,

        favorite: true,

        cover: "images/all-the-light.jpg",

        keywords: [
            "Pulitzer Prize",
            " WWII(double point de vue: français et allemand)",
            " aveugle?"
        ],

        description:
            "On suit en parallèle, une jeune fille parisienne aveugle dont le père gère la collection des pierres rares du Muséum d'Histoire Nat. au début de l'occupation et d'un jeune allemand, génie de la radio, qui se retrouve à s'enroler dans le nazisme pour échapper à la giga misère. Existe en mini-série sur Netflix qui fait bien l'affaire!!",

        date:
            "Lu en Juillet 2026"
    },


    {
        id: 6,

        title: "Atmosphere",

        author: "Taylor Jenkins Reid",

        genre: "Roman",

        rating: 4,

        favorite: true,

        cover: "images/atmosphere.jpg",

        keywords: [
            "Espace (genre fusées)",
            " Love story",
            " Autrice de 7 husbands de Evelyn Hugo"
        ],

        description:
            "l'histoire d'une femme dans les années 80 ou quoi qui rêve de devenir astronaute. et le devient. duh. et elle est aussi amoureuse d'une autre louloute!! ",

        date:
            "Lu en Juillet 2026"
    },

    {
        id: 7,

        title: "Carrie Soto is Back",

        author: "Taylor Jenkins Reid",

        genre: "Roman",

        rating: 3.5,

        favorite: true,

        cover: "images/carrie-soto.jpg",

        keywords: [
            "Tennis (genre le sport)",
            " à peu près la lifestory de serena williams",
            " Autrice de 7 husbands de Evelyn Hugo"
        ],

        description:
            "l'histoire d'une tennis woman qui a pris sa retraite après une blessure et décide de revenir jouer alors qu'elle est un peu '''âgée'''. focus dans le monde du tennis et des sacrifices à faire pour la gloire",

        date:
            "Lu en Juin 2026"
    },



    {
        id: 8,

        title: "The Underground Railroad",

        author: "Colson Whitehead",

        genre: "Roman",

        rating: 5,

        favorite: true,

        cover: "images/underground.jpg",

        keywords: [
            "Esclavage",
            " Réseau souterrain Sud-Nord ",
            " Pulitzer Prize"
        ],

        description:
            "wow. un récit sur un réseau de chemin de fer souterrain qui permettait aux esclaves du Sud de s'enfuir vers le Nord, où ils seraient 'libres'. Dur parce que pas tout rose logique, mais très intéressant pour en apprendre plus sur ce système impressionant",

        date:
            "Lu en juin 2026"
    },


    {
        id: 10,

        title: "American Dirt",

        author: "Jeanine Cummins",

        genre: "Roman",

        rating: 5,

        favorite: true,

        cover: "images/american-dirt.jpg",

        keywords: [
            "Immigration Mexique - USA",
            " Vrai pageturner, et intense et eye-opening",
            " TW: il se passe parfois des choses un peu horribles sur le chemin de la migration"
        ],

        description:
            "Histoire d'une mère et son fils qui doivent s'enfuir du Mexique à cause d'un cartel qui veut leur peau. On suit leur chemin vers ce qu'ils espèrent être la liberté aux USA. C'est - . J'ai déjà réussi à le faire lire à Marius!",

        date:
            "Lu en juin 202 sur les conseils de Madame MacDonald"
    },

    /*
     {
        id: 10,

        title: "",

        author: "",

        genre: "Roman",

        rating: ,

        favorite: true,

        cover: "images/.jpg",

        keywords: [
            " ",
            " ",
            " "
        ],

        description:
            "",

        date:
            "Lu en  2026"

    },

     {
        id: 10,

        title: "",

        author: "",

        genre: "Roman",

        rating: ,

        favorite: true,

        cover: "images/.jpg",

        keywords: [
            " ",
            " ",
            " "
        ],

        description:
            "",

        date:
            "Lu en  2026"

    },

     {
        id: 10,

        title: "",

        author: "",

        genre: "Roman",

        rating: ,

        favorite: true,

        cover: "images/.jpg",

        keywords: [
            " ",
            " ",
            " "
        ],

        description:
            "",

        date:
            "Lu en  2026"

    },

     {
        id: 10,

        title: "",

        author: "",

        genre: "Roman",

        rating: ,

        favorite: true,

        cover: "images/.jpg",

        keywords: [
            " ",
            " ",
            " "
        ],

        description:
            "",

        date:
            "Lu en  2026"

    },


     {
        id: 8,

        title: "",

        author: "",

        genre: "Roman",

        rating: ,

        favorite: true,

        cover: "images/.jpg",

        keywords: [
            " ",
            " ",
            " "
        ],

        description:
            "",

        date:
            "Lu en  2026"
    },
     {
        id: 8,

        title: "",

        author: "",

        genre: "Roman",

        rating: ,

        favorite: true,

        cover: "images/.jpg",

        keywords: [
            " ",
            " ",
            " "
        ],

        description:
            "",

        date:
            "Lu en  2026"
    },

*/

];
